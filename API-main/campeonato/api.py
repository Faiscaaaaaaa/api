...
# Nota: a UI do Swagger/OpenAPI fica pública e acessível em /api/docs/.
# Os endpoints que exigem chave continuam protegidos com `auth=AuthAPIKey()`.
api = NinjaAPI(
    title="Campeonato",
...
# === Autenticação via API Key (Django Ninja) ===
...
# Se quiser proteger toda a API por defeito, substitua a instancia `api` por:
# api = NinjaAPI(auth=AuthAPIKey())
