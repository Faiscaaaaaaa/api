...
def pagina_colega(request):
    """Exemplo simples que chama a API externa usando a chave no header."""
    ...
    def _filtrar_dados(dados, search_text, ordering_value):
        ...
        if ordering_value:
            ...
            if campo == "id":
                lista = sorted(lista, key=lambda item: item.get("id", 0), reverse=reverse)
...
def projetos_list(request):
    """Lista recursos da API externa; suporta ordenação e pesquisa via query params."""
    ordering = request.GET.get("ordering")
    search = request.GET.get("search")
    params = {}
    if search:
    ...
    try:
        ...
        if isinstance(itens, list):
            ...
            if ordering and ordering.lstrip("-") == "id":
                ...
                itens = sorted(itens, key=lambda item: item.get("id", 0), reverse=reverse)
        erro = None
    except requests.exceptions.RequestException as e:
        itens = []
        erro = str(e)
